...
class AgentDQN(AgentBase):
    """
    update process: Q(s,a) <- Q(s,a) + alpha*(reward + gamma * max Q(s',a) - Q(s,a) )
    target：  gamma * max Q(s',a)
    logit:    Q(s,a)
    loss = mse(target - logit)
    ...
    def select_action(self, state) -> int:  # for discrete action space
        if rd.rand() < self.explore_rate:  # epsilon-greedy
            ...
            states = torch.as_tensor((state,), dtype=torch.float32, device=self.device).detach_()
            action = self.act(states)[0]
            a_int = action.argmax(dim=0).cpu().numpy()  # argmaxQ(s',a)
        return a_int
    ...
    def get_obj_critic_raw(self, buffer, batch_size):
        with torch.no_grad():
            ...
            q_label = reward + mask * next_q  # target                      

        q_value = self.cri(state).gather(1, action.type(torch.long)) # logit
