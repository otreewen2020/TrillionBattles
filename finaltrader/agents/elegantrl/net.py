...
#  pytorch 1.5 版本不支持nn.Hardwish() 重新写
...
# 计算Q值，output的维度为action_dim 返回所有action的Q值        
...
class QNetDuel(nn.Module):  
    def __init__(self, mid_dim, state_dim, action_dim):
        ...
        self.net_val = nn.Sequential(nn.Linear(mid_dim, mid_dim), nn.ReLU(),
                                     nn.Linear(mid_dim, 1))          # S value
        self.net_adv = nn.Sequential(nn.Linear(mid_dim, mid_dim), nn.ReLU(),
                                     nn.Linear(mid_dim, action_dim))  # A advantage function value 1
...
# DPG: Deterministic Policy Gradient  Deterministic：输出为action的概率
class Actor(nn.Module): 
    ...
    def forward(self, state):
        return self.net(state).tanh()  # action.tanh() 取值介于（-1，1）

    def get_action(self, state, action_std):
        action = self.net(state).tanh()
        noise = (torch.randn_like(action) * action_std).clamp(-0.5, 0.5)  # 在（-.5，.5）之间截断
        return (action + noise).clamp(-1.0, 1.0) # 添加noise是为了增加探索性
...
class Critic(nn.Module):
    ...
    def forward(self, state, action):
        return self.net(torch.cat((state, action), dim=1))  # Q(s,a) or V(s,a)
...
class NnReshape(nn.Module):
    ...
    def forward(self, x):
        return x.view((x.size(0),) + self.args)  # view函数相当于numpy的reshape


class DenseNet(nn.Module):  # plan to hyper-param: layer_number
    def __init__(self, lay_dim):
        super().__init__()
        self.dense1 = nn.Sequential(nn.Linear(lay_dim * 1, lay_dim * 1), Hardswish())  # Hardswish激活函数
